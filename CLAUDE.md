...
- **Step 4 — DONE**: editing (`src/brainrotbot/edit/`). Per story, muxes the narration WAV onto the silent background clip and appends a standard "follow our page" outro shot, writing `data/final/<post_id>.mp4` and recording `assets.final_video` + `assets.edit`; sets `status="edit_done"`. The background is already exactly the narration's length (Step 3 trims to `assets.audio.duration_sec`), so muxing aligns with no re-trim. With no outro it's a stream-copy mux (`-c:v copy`); with one it's a single ffmpeg `filter_complex` concat (geometry/audio normalized, outro audio used if present else bounded silence). Reuses `_ffmpeg`/`_run`/`probe_duration` from `video/background.py` (the bundled imageio-ffmpeg static binary — never a bare `ffmpeg`). Outro is an optional static asset (`[edit].outro_file`, a 9:16 clip/image in `resources/`); unset/missing → degrades to mux-only. Config in `[edit]` (geometry reused from `[video]`). Use `--skip-edit` to skip. **Deferred**: soft background-music bed (Step 5) and "Follow for part N" splitting of long videos; the ≥1 min target is only *preferred* so no padding is forced. Verified offline (both branches) with real ffmpeg.
- **Steps 5-6 — NOT STARTED**: music, upload, analysis. The numbered sections below are the spec/intent, not status. Each step slots into `pipeline.run()`; `run.bat` stays the stable single entry point.
